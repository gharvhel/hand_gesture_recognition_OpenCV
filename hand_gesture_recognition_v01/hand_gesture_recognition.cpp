...
void find_largest_contour(vector<vector<Point>> &contours, int &largest_contour_index, int &largest_coutour);
int get_quadrant(int &center_x, int &center_y, Mat &image, int &where, char *what_and_where);
...
int main(int argc, char **argv)
{
	...
	Mat image1_mask, image2_mask, combined_image_masks, combined_result, combined_result2, combined_result3;
	...
	findContours(image1_mask, contours1, hierarchy1, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE, Point());
	findContours(image2_mask, contours2, hierarchy2, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE, Point());

	// find largest contour
	int largest_contour1_index = -1, largest_contour2_index = -1;
	int largest_coutour1 = 0, largest_coutour2 = 0;
	find_largest_contour(contours1, largest_contour1_index, largest_coutour1);
	find_largest_contour(contours2, largest_contour2_index, largest_coutour2);
			...
			if (largest_contour1_index == i)
			{
				...
				for (int j = 0; j < defects1[i].size(); j++)
				{
					if (defects1[i][j][3] > 11500)
					{
						...
						if (skiped_first)
						{
							circle(image1, contours1[i][end_point], 40, Scalar(50, 0, 255), -1);
							count1++;
						}
						skiped_first = true;
					}
				...
				quadrant1 = get_quadrant(image1_center_x, image1_center_y, image1, where1, what_and_where);

				// draw contour, convex hull
				// write the where and what
				drawContours(image1, contours1, i, Scalar(130, 0, 75), 10, 8, vector<Vec4i>(), 0, Point());
				drawContours(image1, hullPoint1, i, Scalar(0, 0, 255), 10, 8, vector<Vec4i>(), 0, Point());
				circle(image1, Point(image1_center_x, image1_center_y), 40, Scalar(255, 0, 0), -1, 8, 0);
				putText(image1, what_and_where, Point(image1_width / 3, image1_width / 5), CV_FONT_HERSHEY_SIMPLEX, 4, Scalar(0, 0, 255), 5);
				putText(image1, to_string(count1), Point(image1_width / 5, image1_height / 5), CV_FONT_HERSHEY_SIMPLEX, 4, Scalar(0, 0, 255), 5);
			}
		}
	}
			...
			if (largest_contour2_index == i)
			{
				...
				for (int j = 0; j < defects2[i].size(); j++)
				{
					if (defects2[i][j][3] > 11500)
					{
						...
						if (skiped_first)
						{
							circle(image2, contours2[i][end_point], 40, Scalar(50, 0, 255), -1);
							count2++;
						}
						skiped_first = true;
					}
				...
				quadrant2 = get_quadrant(image2_center_x, image2_center_y, image2, where2, what_and_where);

				// draw contour, convex hull
				// write the where and what
				drawContours(image2, contours2, i, Scalar(130, 0, 75), 10, 8, vector<Vec4i>(), 0, Point());
				drawContours(image2, hullPoint2, i, Scalar(0, 0, 255), 10, 8, vector<Vec4i>(), 0, Point());
				circle(image2, Point(image2_center_x, image2_center_y), 40, Scalar(255, 0, 0), -1, 8, 0);
				putText(image2, what_and_where, Point(image2_width / 3, image2_width / 5), CV_FONT_HERSHEY_SIMPLEX, 4, Scalar(0, 0, 255), 5);
				putText(image2, to_string(count2), Point(image2_width / 5, image2_height / 5), CV_FONT_HERSHEY_SIMPLEX, 4, Scalar(0, 0, 255), 5);
			}
		}
	}
...
void find_largest_contour(vector<vector<Point>> &contours,
						  int &largest_contour_index, int &largest_coutour)
{
